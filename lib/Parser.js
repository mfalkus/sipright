...
exports.parseMessage = (data, ua) =>
{
  ...
  */
  ...
  if (message.hasHeader('content-length'))
  {
    ...
    if (declaredContentLength < availableBodyLength)
    {
      message.validation_warnings.push(`non-fatal: extra body bytes beyond Content-Length (declared=${declaredContentLength}, actual=${availableBodyLength})`);
    }
...
function parseHeader(message, data, headerStart, headerEnd, parserState)
{
  ...
  if (normalizedHeaderName.length > 1)
  {
    ...
    {
      message.validation_warnings.push(`non-fatal: non-canonical header capitalization "${headerName}" (expected "${canonicalHeaderName}")`);
    }
  }
  ...
  if (parserState.singletonHeaders[normalizedHeaderName])
  {
    ...
    {
      message.validation_warnings.push(`non-fatal: duplicate singleton header "${Utils.headerize(headerName)}"`);
      return true;
    }
  }
