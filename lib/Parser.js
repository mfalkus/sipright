...
exports.parseMessage = (data, ua, options) =>
{
  ...
  let message;
  let bodyStart;
  // Be tolerant of "copy/paste" SIP where CRLFs are lost and only LF remains.
  // RFC 3261 requires CRLF, but in practice text exports often normalize line endings.
  const originalData = data;
  const hasCrlf = data.indexOf('\r\n') !== -1;
  const hasLf = data.indexOf('\n') !== -1;

  // Heuristic: only do this normalization for text that looks like a SIP start-line.
  const firstLineEndIdx = hasCrlf ? data.indexOf('\r\n') : (hasLf ? data.indexOf('\n') : -1);
  const firstLineCandidate = firstLineEndIdx !== -1 ? data.substring(0, firstLineEndIdx) : data;
  const looksLikeSipStartLine =
    /\bSIP\/2\.0\b/i.test(firstLineCandidate) &&
    (/^\s*SIP\/2\.0\b/i.test(firstLineCandidate) || /^\s*[A-Z]+\s+.+\s+SIP\/2\.0\b/i.test(firstLineCandidate));

  if (!hasCrlf && hasLf && looksLikeSipStartLine)
  {
    // Missing newline at end-of-input: common when pasting a message without the final line break.
    if (!/\n$/.test(data))
    {
      // We'll fix line endings below; record the warning up-front.
      // (We can't call addValidationWarning() yet; message isn't instantiated until first line parse succeeds.)
    }

    // Normalize lone LF to CRLF to let the SIP header parser operate.
    data = data.replace(/\r?\n/g, '\r\n');
  }
  ...
  message.validation_infos_by_category = {};
  message.validation_infos_detailed = [];

  // If we normalized LF-only input earlier, record that as a non-fatal integrity warning now.
  if (originalData !== data)
  {
    addValidationWarning(message, 'integrity', 'non-fatal: message uses LF line endings (no CRLF); treating LF as CRLF (likely copy/paste/export artifact)');

    if (looksLikeSipStartLine && !/\n$/.test(originalData))
    {
      addValidationWarning(message, 'integrity', 'non-fatal: message is missing trailing newline at end of input (likely copy/paste/export artifact)');
    }
  }

  // If the message ends without the required header terminator (blank line), assume headers end at EOF.
  // This commonly happens when someone copies only the header block but omits the final empty line.
  if (looksLikeSipStartLine && data.indexOf('\r\n\r\n') === -1)
  {
    addValidationWarning(message, 'integrity', 'non-fatal: missing header terminator (blank line); assuming end of headers at end of input');
    data = `${data}\r\n\r\n`;
  }

  // Keep raw message data aligned with any normalization/fixes above.
  message.data = data;
  ...
  */
  ...
  if (message.hasHeader('content-length'))
  {
    ...
    const lineEndingsNormalized = originalData !== data;
    ...
    {
      // If we normalized LF-only input to CRLF for parsing, the declared Content-Length is often based on the
      // original LF bytes. Honoring it after normalization can incorrectly truncate the body (especially SDP).
      if (lineEndingsNormalized && declaredContentLength < availableBodyLength)
      {
        addValidationWarning(message, 'integrity', `non-fatal: Content-Length may be inaccurate due to LF→CRLF normalization (declared=${declaredContentLength}, actual=${availableBodyLength}); using available bytes`);
        message.body = data.substr(bodyStart, availableBodyLength);
      }
      else
      {
  ...
  addRoutingValidationWarnings(message);
  addSdpValidationWarnings(message);

  return message;
};

function addRoutingValidationWarnings(message)
{
  try
  {
    if (!message || typeof message.getHeaders !== 'function' || typeof message.hasHeader !== 'function')
    {
      return;
    }
    ...
    {
      const fromHost = extractSipUriHost(message.getHeader('from'));
      const toHost = extractSipUriHost(message.getHeader('to'));
      if (fromHost)
      {
        contextHosts.push(fromHost);
      }
      if (toHost)
      {
        contextHosts.push(toHost);
      }
    }
    ...
    {
      const contacts = message.getHeaders('contact') || [];
      for (const c of contacts)
      {
        const h = extractSipUriHost(c);
        if (h)
        {
          contactHosts.push(h);
          contextHosts.push(h);
    ...
    const recordRoutes = message.getHeaders('record-route') || [];
    const rrHosts = recordRoutes
      .map(extractSipUriHost)
      .filter(Boolean);
    if (rrHosts.length)
    {
      // Consider Record-Route peers themselves (public RR + private RR mixed).
      contextHosts.push(...rrHosts);
    }

    // Only warn when it looks like a public/private leak (i.e., at least one
    // clearly public IP literal is present somewhere in the routing context).
    // Hostnames are too ambiguous (could be internal DNS), so they don't count.
    const hasPublicIpLiteralContext = contextHosts.some((h) =>
      h && isIpLiteral(h) && !isSuspiciousMediaIp(h)
    );

    if (!hasPublicIpLiteralContext)
    {
      return;
    }

    // Record-Route: warn for private IP literal when mixed context exists.
    const rrPrivateIps = rrHosts
      .filter((h) => isIpLiteral(h) && isSuspiciousMediaIp(h));
    for (const ip of Array.from(new Set(rrPrivateIps)))
    {
      addValidationWarning(
        message,
        'sip',
        `non-fatal: Record-Route contains non-routable address (${ip}) — may break mid-dialog routing across public networks`
      );
    }

    // From/To: warn for private IP literal in URI host when mixed context exists.
    if (typeof message.getHeader === 'function')
    {
      const fromHost = extractSipUriHost(message.getHeader('from'));
      if (fromHost && isIpLiteral(fromHost) && isSuspiciousMediaIp(fromHost))
      {
        addValidationInfo(message, 'sip', `info: From header contains non-routable URI host (${fromHost}) — may reveal private routing/topology information`);
      }

      const toHost = extractSipUriHost(message.getHeader('to'));
      if (toHost && isIpLiteral(toHost) && isSuspiciousMediaIp(toHost))
      {
        addValidationInfo(message, 'sip', `info: To header contains non-routable URI host (${toHost}) — may reveal private routing/topology information`);
      }
    }

    // Contact: warn for private IP literal in URI host when mixed context exists.
    const contactPrivateIps = contactHosts
      .filter((h) => isIpLiteral(h) && isSuspiciousMediaIp(h));
    for (const ip of Array.from(new Set(contactPrivateIps)))
    {
      addValidationWarning(message, 'sip', `non-fatal: Contact header contains non-routable URI host (${ip})`);
    }
...
function addValidationInfo(message, category, info)
{
  if (!message || !info)
  {
    return;
  ...
  if (!Array.isArray(message.validation_infos))
  {
    message.validation_infos = [];
  }
  message.validation_infos.push(info);

  if (!message.validation_infos_by_category)
  {
    message.validation_infos_by_category = {};
  }
  if (!Array.isArray(message.validation_infos_by_category[normalizedCategory]))
  {
    message.validation_infos_by_category[normalizedCategory] = [];
  }
  message.validation_infos_by_category[normalizedCategory].push(info);

  if (!Array.isArray(message.validation_infos_detailed))
  {
    message.validation_infos_detailed = [];
  }
  message.validation_infos_detailed.push({
    category : normalizedCategory,
    info
  });
}
...
function addSdpValidationWarnings(message)
{
  try
  {
    ...
    // SDP is line-oriented (typically CRLF). If we see SDP (or SDP Content-Type) but the payload
    // ends without a newline terminator, it's frequently a truncation/export artifact and can
    // confuse downstream SDP parsers.
    if (!/[\r\n]$/.test(String(body)))
    {
      addValidationWarning(message, 'integrity', 'non-fatal: SDP body is missing final newline (expected line terminator at end of body)');
    }
    ...
    for (const m of media)
    {
      ...
      {
        // Note: sdp-transform uses `payloads: 0` (number) for "m=... 0", which is falsy.
        // Treat 0 as a valid payload list.
        const rawPayloads = (m && Object.prototype.hasOwnProperty.call(m, 'payloads')) ? m.payloads : undefined;
        const payloadsStr = rawPayloads === undefined || rawPayloads === null ? '' : String(rawPayloads).trim();
...
function parseHeader(message, data, headerStart, headerEnd, parserState)
{
  ...
  if (normalizedHeaderName.length > 1)
  {
    ...
    if (normalizedHeaderName.startsWith('x-'))
    {
      ...
      {
        addValidationInfo(message, 'sip', `info: X- extension header should start with "X-" (got "${headerName}")`);
      }
    }
    else
    {
      ...
      {
        addValidationInfo(message, 'sip', `info: non-canonical header capitalization "${headerName}" (expected "${canonicalHeaderName}")`);
      }
    }
  }
...
function isIpLiteral(host)
{
  ...
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(unbracketed) || /^[0-9a-fA-F:]+$/.test(unbracketed);
}

function extractSipUriHost(value)
{
  ...
  // Remove surrounding name-addr angle brackets if present.
  const inside = raw.startsWith('<') ? raw.slice(1, raw.indexOf('>') === -1 ? raw.length : raw.indexOf('>')) : raw;
  const lower = inside.toLowerCase();
  const sipIdx = lower.indexOf('sip:');
  if (sipIdx === -1)
  {
    return '';
  }

  let uri = inside.slice(sipIdx + 4); // after "sip:"
  uri = uri.split(';')[0].split('?')[0].trim();

  // Strip userinfo if present.
  const atIdx = uri.lastIndexOf('@');
  const hostport = atIdx !== -1 ? uri.slice(atIdx + 1) : uri;

  // IPv6 literal host is bracketed: [::1]:5060
...
function extractViaHost(viaValue)
{
  const s = String(viaValue || '').trim();
  if (!s)
  {
    return '';
  }

  // Example: "SIP/2.0/UDP 1.2.3.4:5060;branch=..."
  const m = s.match(/^SIP\/2\.0\/[A-Z]+[\t ]+([^;]+)(?:;|$)/i);
  if (!m)
  {
    return '';
  }

  const sentBy = String(m[1] || '').trim();
  if (!sentBy)
  {
    return '';
  }
  ...
  {
    const end = sentBy.indexOf(']');
    if (end !== -1)
    {
      return sentBy.slice(0, end + 1);
    }
  }

  return sentBy.split(':')[0].trim();
}
